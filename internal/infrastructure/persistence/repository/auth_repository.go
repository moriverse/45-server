...
func (r *AuthRepository) FindByProvider(ctx context.Context, provider auth.Provider, providerUserID string) (*auth.Auth, error) {
	var model models.Auth
	if err := r.db.WithContext(ctx).First(&model, "provider = ? AND provider_user_id = ?", provider, providerUserID).Error; err != nil {
